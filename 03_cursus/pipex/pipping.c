...
/*   Updated: 2024/03/19 18:18:54 by zkepes           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	pipe_from_file(t_data *d)
{
	...
	if (dup2(d->fd_start, d->pipes[0][READ]) == -1)
		e_message_exit(d, "dup2 READ PARENT");
}
...
void	write_outfile(t_data *d)
{
	...
	if (fd_outf == -1)
		e_message_exit(d, "error: open or create output file");
	b_read = true;
	while (b_read > 0)
	{
		b_read = read(d->pipes[d->n_cmd][READ], buffer, BUFFER_SIZE);
		if (write(fd_outf, buffer, b_read) != b_read)
			e_message_exit(d, "error: write to output file");
	}
	if (b_read == -1)
		e_message_exit(d, "error: read bytes from pipe");
	close(fd_outf);
}
